...
public class OrderServiceImpl implements OrderService {
    ...
    /**
     * 추상화에만 의존하는 경우
     * final : null 허용하지 않는 경우. 생성시 초기화해야 함
     * application과 관계 없는 테스트 코드 내에서 사용하는 경우 필드 주입 방식 사용해도 괜찮다.
     */
...
//    private DiscountPolicy discountPolicy; // 외부에서  OrderServiceImpl에 DiscountPolicy 구현체를 주입해줘야 함
    ...
    /**
     * DI: 필드 주입 방법
     * 권장 X : Field injection is not recommended 라는 메세지를 남긴다.
     * 스프링 컨테이너가 아닌 순수한 자바로 테스트 하기 어려움
     */
    @Autowired
    private MemberRepository memberRepository;
    @Autowired
     ...
     * 선택적 의존관계 주입.
     *
     * @Autowired(requeired = false)로 설정하면 의존관계가 없으면 메서드 자체가 호출되지 않는다.
     * 변경 가능성 있는 의존관계에 사용.
     */
//    @Autowired
